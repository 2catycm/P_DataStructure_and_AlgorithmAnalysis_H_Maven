...
class CompleteBinaryTree <Item> extends Tree<Item>{
    protected Item[] heap;
    protected int capacity;
    protected static final int DEFAULT_INITIAL_CAPACITY = 11;
    private void expand() {
        final var newHeap = (Item[]) new Comparable[capacity << 2];
        System.arraycopy(heap, 0, newHeap, 0, capacity+1);
        heap = newHeap;
    }

    @Override
    public TreeNodeIterator<Item> root() {
        return null;
    }

    @Override
    public Tree<Item> subTree(TreeNodeIterator<Item> newRoot) {
        return null;
    }

    @Override
    public Tree<Item> combinedTree(Item newRootData, LinkedList<Tree<Item>> subTrees) {
        return null;
    }
}
